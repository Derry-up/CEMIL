...
class REGRESSOR:
    def __init__(self, _train_X, _train_Y, data_loader, _nclass, _cuda, seedinfo, train_base=False, _lr=0.001, _beta1=0.5, _nepoch=20, _batch_size=100, _embed_dim=1000, _num_layers=3, opt=None):
        ...
        self.lr = _lr
        self.beta1 = _beta1
        self.optimizer_classifier = optim.Adam(self.model.parameters(
        ), lr=self.opt.classifier_lr, betas=(self.opt.classifier_beta1, 0.999))
        ...
        if not train_base:
            best_model = torch.load(f'{self.opt.rootpath}/models/base-classifiers/{self.opt.dataset}res101_finetuned_seed{self.seedinfo}_clr{self.opt.classifier_lr}_nep{self.opt.classifier_nepoch}')
    ...
    def pred_weights_and_val(self, weight_model):
        ...
        for n, attribute_vector in enumerate(attributes_to_regress):
            ...
            self.unseen_model.fc.weight.data[n,
                                             :] = pred_weights[:self.input_dim]
            self.unseen_model.fc.bias.data[n] = pred_weights[self.input_dim]
            self.ext_model.fc.weight.data[len(
                self.seenclasses)+n, :] = pred_weights[:self.input_dim]
    ...
    def val_model(self, model, test_X, test_label, target_classes, calc_entropy=False):
        ...
        if self.opt.save_pred_matrix:
            ...
            torch.save(prediction_matrix, f'{self.opt.rootpath}/outputs/pred_matrix{self.opt.dataset}res101_finetuned_len_test_' + str(len(test_X)) + '_len_tar_' + str(len(target_classes)) + '.pt')
        if calc_entropy:
    ...
    def val_gzsl(self):
        if self.opt.norm_scale_heuristic:
            pred_weights = torch.cat((self.unseen_model.fc.weight.data, torch.unsqueeze(
                self.unseen_model.fc.bias.data, 1)), 1)
            pred_weights = pred_weights / \
                (10*torch.norm(pred_weights, dim=1).mean())
            self.unseen_model.fc.weight.data[:,
                                             :] = pred_weights[:, :self.input_dim]
            self.unseen_model.fc.bias.data[:] = pred_weights[:, self.input_dim]
            self.ext_model.fc.weight.data[len(
                self.seenclasses):, :] = pred_weights[:, :self.input_dim]
            self.ext_model.fc.bias.data[len(
                self.seenclasses):] = pred_weights[:, self.input_dim]

        acc_gzsl = self.val_model(self.ext_model, torch.cat((self.test_seen_feature, self.test_unseen_feature), 0),
                                  torch.cat((data_preparation.map_label(self.test_seen_label, self.seenclasses), data_preparation.map_label_extend(
                                      self.test_unseen_label, self.unseenclasses, self.seenclasses)), 0),
                                  torch.cat((data_preparation.map_label(self.seenclasses, self.seenclasses), data_preparation.map_label_extend(
                                      self.unseenclasses, self.unseenclasses, self.seenclasses)), 0),
                                  calc_entropy=self.calc_entropy)
        acc_seen = self.val_model(self.ext_model, self.test_seen_feature, data_preparation.map_label(
            self.test_seen_label, self.seenclasses), data_preparation.map_label(self.seenclasses, self.seenclasses))
        acc_unseen = self.val_model(self.ext_model, self.test_unseen_feature, data_preparation.map_label_extend(
            self.test_unseen_label, self.unseenclasses, self.seenclasses), data_preparation.map_label_extend(self.unseenclasses, self.unseenclasses, self.seenclasses))
        H = 2*acc_seen*acc_unseen / (acc_seen+acc_unseen)
        # ZSL
        acc_unseen_zsl = self.val_model(self.unseen_model, self.test_unseen_feature, data_preparation.map_label(
            self.test_unseen_label, self.unseenclasses), data_preparation.map_label(self.unseenclasses, self.unseenclasses))
